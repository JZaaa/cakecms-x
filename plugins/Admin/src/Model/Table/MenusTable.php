...
//    }
    ...
    public function getMenuForManage($group_id = null){
        $roleMenu = TableRegistry::get('Admin.Roles')->getData(array('Roles.id' => $group_id));
        $menus = json_decode($roleMenu->menus);
        $menuData = (object)null;
        if (!empty($menus)) {
            $menuData = $this->find('threaded')
                ->where(['Menus.id in'=>$menus,'Menus.isshow'=>1])
                ->order(['Menus.sort'=>'desc','Menus.id'=>'asc']);
        }
        return $menuData;
    }
}
